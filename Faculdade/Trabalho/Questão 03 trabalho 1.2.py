...
while True:
    try:
        ...
        continue
    if len (ip_dec) != 4 or len (ip_bin_32) != 32 or int(cidr) < 8 or int(cidr) > 32:
        print('Erro! Digite o IP no formato solicitado.')
        continue
    else: break
...
print('O total de hosts válidos é: {}'.format(total_hosts))
print('O endereço broadcast é:     {}.{}.{}.{}'.format(int(broadcast_0,2), int(broadcast_1,2), int(broadcast_2,2), int(broadcast_3,2)))
print('Os endereços IPs válidos para cada subrede estão entre {}.{}.{}.{} e {}.{}.{}.{}'.format(int(end_rede_0,2), int(end_rede_1,2), int(end_rede_2,2), int(end_rede_3,2)+1,int(broadcast_0,2), int(broadcast_1,2), int(broadcast_2,2), int(broadcast_3,2)-1))
